#!/bin/bash
# =============================================================================
# OpenTracker - Complete Server Installation Script
# =============================================================================
# This script installs and configures OpenTracker from scratch on a fresh server
...
# CONFIGURATION
# =============================================================================
INSTALL_DIR="${INSTALL_DIR:-/opt/opentracker}"
REPO_URL="${REPO_URL:-https://github.com/florianjs/opentracker.git}"
BRANCH="${BRANCH:-main}"
...
preflight_checks() {
    ...
    if [[ $TOTAL_RAM -lt 1500 ]]; then
        log_warn "System has less than 2GB RAM. OpenTracker may run slowly."
...
create_env_file() {
    ...
    cat > "$ENV_FILE" << EOF
# =============================================================================
# OpenTracker Production Environment Configuration
...
# =============================================================================
setup_repository() {
    log_step "Setting up OpenTracker repository..."
...
# =============================================================================
build_application() {
    log_step "Building OpenTracker Docker image..."
    
    cd "$INSTALL_DIR"
    
    # Build the image
    docker build -t opentracker:latest .
...
# =============================================================================
start_services() {
    log_step "Starting OpenTracker services..."
    
    cd "$INSTALL_DIR"
    
    # Check for existing postgres volume with potentially old credentials
    if docker volume ls -q | grep -q "opentracker_postgres_data"; then
...
[Unit]
Description=OpenTracker P2P Tracker
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}║              OpenTracker Installation Complete!                   ║${NC}"
    echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo -e "  Restart services: ${CYAN}systemctl restart opentracker${NC}"
    echo -e "  Stop services:    ${CYAN}systemctl stop opentracker${NC}"
    ...
    cat > "$CREDS_FILE" << EOF
================================================================================
OpenTracker Installation Credentials
Generated: $(date)
================================================================================
...
# =============================================================================
uninstall() {
    log_step "Uninstalling OpenTracker..."
    
    read -p "Are you sure you want to uninstall OpenTracker? This will delete ALL data! (type 'yes' to confirm): " CONFIRM
    if [[ "$CONFIRM" != "yes" ]]; then
    ...
    # Remove Docker image
    docker rmi opentracker:latest 2>/dev/null || true
    
    # Remove systemd service
    systemctl stop opentracker 2>/dev/null || true
    systemctl disable opentracker 2>/dev/null || true
    rm -f /etc/systemd/system/opentracker.service
    systemctl daemon-reload
    ...
    log_success "OpenTracker uninstalled"
}
...
case "${1:-}" in
    --uninstall|-u)
        ...
        ;;
    --help|-h)
        echo "OpenTracker Installation Script"
        echo ""
        echo "Usage: $0 [options]"
        echo ""
        echo "Options:"
        echo "  --help, -h        Show this help message"
        echo "  --uninstall, -u   Uninstall OpenTracker"
        echo ""
        echo "Environment Variables:"
        echo "  INSTALL_DIR       Installation directory (default: /opt/opentracker)"
